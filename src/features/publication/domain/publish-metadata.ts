...
const csvRessource = (output: Output, date: Date, suffix?: string): PublishRessource => ({
...
const jsonRessource = (output: Output, date: Date, suffix?: string): PublishRessource => ({
  source: `${output.path}/${mediationNumeriqueFileName(
    date,
    output.name.toLowerCase().replace(/\s/g, '-'),
    output.territoire.toLowerCase().replace(/\s/g, '-'),
...
export const generatePublishMetadata = (
  ...
  ressources: [csvRessource(output, date, suffix), jsonRessource(output, date, suffix)]
});
