...
type FixedAdresse = Source | undefined;
...
const voieField = (source: Source, voie: Jonction & Partial<Colonne>): string =>
  isColonne(voie)
    ? source[voie.colonne] ?? ''
...
const applyCleanOperation =
  (recorder: Recorder) =>
  (cleanOperation: CleanOperation, valueToFix: string | undefined, source: Source): Source =>
...
const fixAndRetry =
  (recorder: Recorder) =>
  (source: Source, matching: LieuxMediationNumeriqueMatching, error: unknown): Adresse =>
...
export const processAdresse =
  (recorder: Recorder) =>
  (source: Source, matching: LieuxMediationNumeriqueMatching): Adresse => {
    try {
