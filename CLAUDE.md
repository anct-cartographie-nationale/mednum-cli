...
- **Un identifiant absent se dérive du contenu, jamais de l'index de ligne.** `empreinteDuContenu` prend `nom + commune + adresse`, chaque partie normalisée, et en rend une empreinte courte. L'index changeait dès que le producteur réordonnait son fichier, et un identifiant qui bouge casse les liens qui le citent. Le repli vaut pour une source sans colonne d'identifiant — Paca, 130 lieux — **comme pour une ligne dont la colonne est vide** : cinquante-trois lieux de Loire-Atlantique partageaient l'identifiant `Loire-Atlantique_`, un doublon que le modèle `Id` ne peut pas voir puisqu'il n'exige qu'une lettre ou un chiffre.
- **L'empreinte se calcule sur l'adresse de la source, jamais sur celle qui est publiée.** La seconde vient de la Base Adresse Nationale, qui révise ses réponses au fil de son référentiel, de l'état du cache et de la qualité du lot : mesuré le 21 septembre, neuf adresses publiées sur cinquante-trois ont changé en une après-midi, aucune adresse de source en cinq jours. C'est pourquoi `processId` reçoit la ligne brute et non la ligne enrichie par le géocodage.
- **Deux lieux d'une même source qui partagent un identifiant sont écartés tous les deux.** `sansIdentifiantEnDouble` les retire avant l'écriture, motif au rapport. Publier le premier venu ferait disparaître l'autre en silence, et fabriquer un discriminant masquerait au producteur un problème qui lui appartient.
- **Le cache d'adresses se consulte par index, jamais par balayage.** `indexerParEtiquette` le construit une fois par exécution et `AddressIndex` circule ensuite jusqu'au géocodage par lot. Chercher par `find` ou `some` coûtait une lecture complète du cache par adresse traitée — quinze mille entrées parcourues vingt mille fois — au point qu'une exécution sans cache allait plus vite qu'une exécution avec.
...
- **La proportion de lignes muettes ne se juge que sur les adresses jamais tentées.** Un lot ne contient jamais d'adresse déjà résolue — `needsGeocoding` les écarte — mais il mêle des adresses neuves et des échecs rejoués après sept jours. Ces derniers forment un tamis de cas difficiles : rejoués, ils rendent de 27 % à 56 % de lignes muettes, ce qui est normal et ne dit rien du géocodeur. Les compter ferait déclarer dégradé tout lot de rattrapage, qui ne s'inscrirait alors jamais au cache et reviendrait chaque nuit sans jamais se stabiliser — l'inverse exact du cliquet que le cache doit produire. Seules les premières tentatives forment un échantillon non biaisé ; les adresses rejouées subissent le verdict sans entrer dans son calcul.
